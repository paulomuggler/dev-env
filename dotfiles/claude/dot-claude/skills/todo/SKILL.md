...
5. `## Verify Report` — executor's verification results (Phase 3b)
6. `## Human Validation` — human-facing checklist (generated by fresh subagent, Phase 3c)
7. `## Work Report` — executor's completion summary (Phase 4)
...
#### Phase 3b: Verify Execute (executor)

The executor runs the checks from the verify plan and fixes any failures.

15. Read the `## Verify Plan` section from the task file
16. **Execute each item**, checking them off as they pass. For Playwright verification:
...
17. **Append `## Verify Report`** documenting results (checked items with evidence)
18. If any items fail → fix the issue, commit the fix (code-only commit), re-verify failed items only
19. Reset retry counter (`.claude/todo-loop-retries`) on successful verification

#### Phase 3c: Human Validation (fresh subagent)

After agent verification passes, generate a human validation checklist using a **fresh subagent**.

20. **Spawn a `general-purpose` subagent** with `model: "sonnet"`:
    ```
...
21. **Update state file:** Set `phase: complete`
22. **Work report:** Append a `## Work Report` section to the task file documenting:
