...
4. `## Verify Plan` — agent's planned verification steps
5. `## Verify Report` — agent's self-verification results
6. `## Human Validation` — human-facing checklist (generated by a fresh subagent, see Phase 3b)
7. `## Work Report` — executor's completion summary
...
#### Phase 3b: Human Validation Script

After agent verification passes, generate a human validation checklist using a **fresh subagent** (separate reasoning context from the executor — avoids shared blind spots).

17. **Spawn a `general-purpose` subagent** with `model: "sonnet"`:
    ```
...
18. The subagent writes the section directly to the task file. No further action needed from the executor.

#### Phase 4: Complete

19. **Update state file:** Set `phase: complete`
20. **Work report:** Append a `## Work Report` section to the task file documenting:
...
21. Set `status: done`, `updated` to today in the task file frontmatter
22. **Notify completion:** Send OS notification via Bash:
    ```bash
...
24. **Task tracking commit:** Commit all `.agents/TODO/` changes (work report, human validation, status, INDEX.md) with `[todo]` prefix
25. **Update state file:**
    - If mode is `single`: delete `.agents/TODO/.work-state`
    - If mode is `loop` or priority (`P0`, `P1`, etc.) **without `auto-clear`**: clear task field, pick next eligible task (respecting `filter` if set), continue to step 26
...
26. In loop/priority modes (without `auto-clear`): repeat pick → plan → execute → verify → complete until no eligible tasks remain (respecting priority and tag filters if set), then report summary
...
- **Enter complete:** Code commit any verify-phase fixes. Task tracking commit: verify report, human validation, work report, status → done, INDEX.md regenerated.
